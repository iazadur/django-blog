...
def pass_change(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            changed = True
    return render(request, 'App_Login/change_pass.html', {'form': form, 'changed': changed})
...
def cng_pro_pic(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('App_Login:profile'))
    return render(request, 'App_Login/cng_pro_pic.html', {'form': form})